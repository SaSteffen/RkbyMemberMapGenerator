"""Training-cluster detection (FR-007, data-model.md § Training Cluster,
research.md §7): groups of 3+ current-season Riders (any experience level)
living close enough together to plausibly train together, via the
generalized `find_overlap_groups`."""
...
def find_training_clusters(
    ...
    never end up "in" a cluster (Acceptance Scenario 2.3). Two pool members
    are linked when `haversine_km(a, b) <= cluster_radius_km`; a cluster is
    one connected component of at least 3 linked members."""
    positions = {
