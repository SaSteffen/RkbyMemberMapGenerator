...
A record's `alias_match_keys` folds an older season's differently-computed
key for the same person into this one before grouping, so a name-derived
key change somewhere in their history doesn't split them into two entries."""
...
def merge_seasons(
    ...
    for season_label, records in eligible_by_season.items():
        for record in records:
            key = canonical_match_key.get(record["match_key"], record["match_key"])
            records_by_match_key.setdefault(key, {})[season_label] = record
