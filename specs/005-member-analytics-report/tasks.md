...
data shown as an explicit "unknown" category rather than dropped — plus, within the
notebook only (never the export), a data-gap list naming which members are missing
which field, by `match_key` (FR-017).

**Independent Test**: Point the report at one season's synthetic data and confirm the
resulting counts and charts match a manual count of that season's fixture records, and
that a fixture member missing a field (no birthday/sex/coordinates) correctly appears
in the notebook's data-gap list under that field, by `match_key`.
...
file, landing outside the git repo, containing aggregates only and never the FR-017
data-gap list.
...
chart/table visible, no per-member roster, and no trace of the T019 data-gap cell; and
that nothing new appears under `git status` in the repo.
...
- **User Stories (Phase 3-6)**: All depend on Foundational completion.
  - US1 (P1): No dependency on US2/US3/US4. `data_gaps()` (T013/T016/T019) depends only
    on the Foundational frame (T011), same as `season_summary()`.
  ...
  - US4 (P3): Its own code (`ensure_reports_dir_and_gitignore`) has no functional
    ...
    cells US1-3 add, and correctly excluding the data-gap list requires T019's
    `remove-cell` tag already being in place — implemented last.
...
- Within US1: T012 and T014 (different files) run in parallel; T013 shares
  `tests/unit/test_rkby_report_aggregate.py` with T012, so it follows T012 rather than
  running alongside it.
- Within US2: T020 and T021 (different files) run in parallel.
- Within US3: T025 and T026 (different files) run in parallel; T027/T028 (both in
  `aggregate.py`) and T029 (in `plots.py`) are sequential implementation steps.
- T034 and T035 (Polish) run in parallel — different concerns, no shared file.
...
Task: "retention_cohort/retention_by_split tests in tests/unit/test_rkby_report_aggregate.py (T025)"
Task: "Retention chart-builder tests in tests/unit/test_rkby_report_plots.py (T026)"
```
...
4. **STOP and VALIDATE**: run quickstart.md Scenarios 1 and 1a against synthetic data —
   role/gender/age/distance breakdown for one season, with "unknown" categories showing
   correctly, and the FR-017 data-gap list correctly naming the fixture member(s)
   missing a field, by `match_key`.
5. This is a usable deliverable on its own.

### Incremental Delivery

1. Setup + Foundational → foundation ready.
2. Add US1 → validate independently → MVP (single-season snapshot + data-gap list).
...
5. Add US4 → validate independently (export to a shareable HTML file, outside the
   repo, aggregates only, data-gap list excluded).
...
- The `remove-cell` tag T019 adds is only as strong as the export command actually
  using `--TagRemovePreprocessor` (T033) — don't treat either task as done without the
  other; T035's quickstart walkthrough is what verifies the pair actually works together.
