...
## 1. Overall architecture: a small offline SPA, generated by a Python script

**Decision**: A two-part deliverable. (a) `scripts/generate_interactive_map.py` — a
pure-Python script (no new Python dependencies) that merges/geocodes/bundles data
exactly like `generate_member_maps.py` does today, then assembles the final shared
artifact. (b) `frontend/interactive-map/` — a small Vite + Vitest + Leaflet
single-page app, source-controlled and **built ahead of time, with its build output
(`dist/`) committed to git** like any other vendored asset (comparable to
`scripts/rkby_maps/assets/rynke.png`). The Python script never invokes `npm`/`vite`
at generation time; it only reads the already-built `dist/index.html`, writes a
small generated data file next to it, and copies per-run binaries (photos, basemap
image) alongside. A normal `uv run scripts/generate_interactive_map.py` run needs
Node/npm installed on *nobody's* machine — only a contributor editing the frontend's
`src/` needs Node, the same way only a contributor editing Python needs `uv`.

**Rationale**: The user explicitly requested Vite + Vitest for the frontend
("i think this will be an spa anyway, so we will use vite and vitest, right?"),
which also resolves the open Principle V (test-first) question for the JS layer —
Vitest gives real red-green unit tests for the app's pure logic (§5, §6, §7) with
zero extra config, since it ships as Vite's companion test runner. Splitting build
(dev-time, Node) from generation (run-time, Python-only) keeps the constitution's
"Language: Python 3" framing intact for the actual *script* Principle II cares
about, and keeps `generate_interactive_map.py` exactly as dependency-light and
network-optional-except-for-geocoding/tiles as `generate_member_maps.py` already
is — a contributor who never touches the frontend never needs Node at all.

**Alternatives considered**: Hand-rolled vanilla JS with manually vendored Leaflet
files and no build step (rejected once the user asked for Vite/Vitest — also would
have left the JS logic in §5–§7 with no test coverage, a real gap against
Principle V); having the Python script shell out to `npm run build` on every
generation run (rejected — adds a Node/npm runtime requirement to the one workflow
the constitution most wants to stay simple and dependency-light, for no benefit
over "rebuild once when the frontend changes, commit the output").
