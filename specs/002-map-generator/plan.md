...
|---|---|---|
| I. Member Data Privacy First | Name, photo, phone, and birthday are never transmitted anywhere. **Flagged**: address *text* is sent to Nominatim (a third-party service) to geocode it — literally in tension with this NON-NEGOTIABLE principle's unqualified "MUST NOT be uploaded to third-party cloud services" wording. spec.md's Assumptions section already documents this as a deliberate, narrowly-scoped exception (address only, once per address ever, cached locally afterward — see research.md §3), but a spec's Assumptions section cannot by itself amend a NON-NEGOTIABLE principle. See Complexity Tracking below. Generated maps themselves stay local (written under `RKBY_DATA_DIR`, gitignored) — this feature doesn't add any new sharing/upload path for the *maps*, only for the one-time address lookup that makes them possible. Excluded/ignored members are already left off every map (FR-003/FR-004), supporting the opt-out requirement. | **CONDITIONAL PASS — see Complexity Tracking** |
...
**Complexity Tracking entry required for Principle I** — see below; this is not a
silent override, it's a documented exception the constitution should formally ratify
(recommend running `/speckit-constitution` to add it) before/alongside implementation.
...
Phase 0. All gates above still hold at their Phase-0 status; the Principle I exception
remains open pending the constitution-amendment recommendation, not something Phase 1
design could itself resolve.
